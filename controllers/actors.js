...
const { OpenAI } = require('openai');  // Note the curly braces

// Add error handling for API key initialization
...
exports.getActor = async (req, res, next) => {
    ...
    try {
        ...
        // Fetch the relevant posts (script feed) for the actor
        ...
        // Generate dynamic responses for each post using the OpenAI API
        for (let script of script_feed) {
            const postText = script.text; // Assuming the post content is in `script.text`
            const prompt = `Generate a thoughtful and engaging comment based on the following post: "${postText}"`;
            ...
            const aiComment = openaiResponse.data.choices[0].text.trim();
            
            // Attach the AI-generated comment to the post
            script.aiComment = aiComment;
        }
