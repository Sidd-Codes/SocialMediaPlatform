        ...
        if (req.user) {
            ...
            for (const notification of notification_feed) {
                ...
                if (notification.userPostID >= 0) {
                    ...
                    if (notification.time <= time_diff) {
                        if (notification.notificationType == "reply") {
                            ...
                            if (notifyIndex == -1) {
                                ...
                                if (notification.notificationType == 'harmful') {
                                    tmp.numHarmfuls = 1
                                }
                                ...
                                // Update existing notification counts.
                                if (notification.notificationType == 'like') {
                                    final_notify[notifyIndex].numLikes += 1;
                                }
                                if (notification.notificationType == 'harmful') {
                                    final_notify[notifyIndex].numHarmfuls += 1;
                                }
                                //Update notification like count.
                                if (notification.notificationType == 'share') {
                                    final_notify[notifyIndex].numShares += 1;
                                }
                            ...
                            //Update the number of likes or Share or Harmful on user post
                            if (notification.notificationType == 'like') {
                            ...
                            if (notification.notificationType == 'harmful') {
                                const postIndex = _.findIndex(user.posts, function(o) { return o.postID == userPostID; });
                                user.posts[postIndex].harmfuls = final_notify[notifyIndex].numHarmfuls;
                            }
                                                        //Update the number of likes on user post
                            if (notification.notificationType == 'share') {
                                const postIndex = _.findIndex(user.posts, function(o) { return o.postID == userPostID; });
                                user.posts[postIndex].shares = final_notify[notifyIndex].numShares;
                            }
                        } //end of LIKE or READ or Share or Harmful
                    } //end of userPost (read, like, comment, share, harmful)
                } //Notification is about a userReply (read, like, harmful)
                    ...
                    //check if we show this notification yet
                    if (notification.time <= time_diff) {
                        const key = "reply_" + notification.notificationType + "_" + userReplyID; //reply_like_X, reply_read_X, reply_share_X, reply_harmful_X
                        ...
                        if (notification.notificationType == 'share') {
                            if (postType == "user") {
                                const postIndex = _.findIndex(user.posts, function(o) { return o.postID == userPostID; });
                                //const commentIndex = _.findIndex(user.posts[postIndex].comments, function(o) { return o.commentID == userReplyID && o.new_comment == true });
                                user.posts[postIndex].comments[commentIndex].shares = final_notify[notifyIndex].numShares;
                            } else {
                                const postIndex = _.findIndex(user.feedAction, function(o) { return o.post.equals(userPostID); });
                                //const commentIndex = _.findIndex(user.feedAction[postIndex].comments, function(o) { return o.new_comment_id == userReplyID && o.new_comment == true });
                                user.feedAction[postIndex].comments[commentIndex].shares = final_notify[notifyIndex].numShares;
                            }
                        }
                        if (notification.notificationType == 'harmful') {
                            if (postType == "user") {
                                const postIndex = _.findIndex(user.posts, function(o) { return o.postID == userPostID; });
                                const commentIndex = _.findIndex(user.posts[postIndex].comments, function(o) { return o.commentID == userReplyID && o.new_comment == true });
                                user.posts[postIndex].comments[commentIndex].shares = final_notify[notifyIndex].numHarmfuls;
                            } else {
                                const postIndex = _.findIndex(user.feedAction, function(o) { return o.post.equals(userPostID); });
                                const commentIndex = _.findIndex(user.feedAction[postIndex].comments, function(o) { return o.new_comment_id == userReplyID && o.new_comment == true });
                                user.feedAction[postIndex].comments[commentIndex].shares = final_notify[notifyIndex].numHarmfuls;
                            }
                        }
                    }
                }
