...
async function getDataExport() {
    ...
    const csvWriter_header = [
        ...
        { id: 'NumActorPostsHarmful', title: 'NumActorPostsHarmful' },
        { id: 'NumActorPostsShared', title: 'NumActorPostsShared' },
        { id: 'NumActorCommentsLiked', title: 'NumActorCommentsLiked' },
        { id: 'NumActorCommentsHarmful', title: 'NumActorCommentsHarmful' },
        ...
        { id: 'ActorPostsHarmful', title: 'ActorPostsHarmful' },
        { id: 'ActorCommentsLiked', title: 'ActorCommentsLiked' },
        { id: 'ActorCommentsHarmful', title: 'ActorCommentsHarmful' },
    ...
    for (const user of users) {
        ...
        for (const feedAction of user.feedAction) {
            ...
            if (feedAction.harmful) {
                NumActorPostsHarmful++;
                ActorPostsHarmful.push(feedAction.post.postID);
            }
            ...
            const CommentsHarmful_list = feedAction.comments.filter(comment => !comment.new_comment && comment.harmful);
            NumActorCommentsHarmful += CommentsHarmful_list.length;
            if (NumActorCommentsHarmful > 0) {
                for (const harmfulComment of CommentsHarmful_list) {
                    ActorCommentsHarmful.push(feedAction.post.comments.find(comment => harmfulComment.comment.equals(comment._id)).commentID);
                }
            }
        }
