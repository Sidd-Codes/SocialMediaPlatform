...
const userSchema = new mongoose.Schema({
    ...
    numPostShares: { type: Number, default: 0 }, // Indicates the # of actor posts shared. Count begins at 1.

    numPostHarmfuls: { type: Number, default: 0 }, // Indicates the # of actor posts harmfuled. Count begins at 1.
    numCommentHarmfuls: { type: Number, default: 0 }, // Indicates the # of actor comments harmfuled. Count begins at 1.
    ...
    posts: [new Schema({
        ...
        shared: { type: Boolean, default: false }, // Indicates if the user has shared the post
        shares: { type: Number, default: 0 }, // Indicates the number of shares on the post by actors (excludes the user's own like)
        harmful: { type: Boolean, default: false }, // Indicates if the user has harmful the post
        harmfuls: { type: Number, default: 0 }, // Indicates the number of harmful on the post by actors (excludes the user's own like)
        ...
        comments: [new Schema({
            ...
            harmful: { type: Boolean, default: false }, // Indicates if the user has hamful the comment
            harmfuls: { type: Number, default: 0 } // Indicates the # of likes on the comment by actors (excludes the user's own like)
        }, { versionKey: false })],
    ...
    postStats: { // Statistics about the user
        ...
        GeneralPostShares: Number, // # of posts shares
        GeneralPostHarmfuls: Number, // # of posts shares
        GeneralCommentHarmfuls: Number, // # of comments shares
    ...
    feedAction: [new Schema({
        ...
        liked: { type: Boolean, default: false }, // Indicates if the user has liked the post
        shared: { type: Boolean, default: false }, // Indicates if the user has shared the post
        harmful: { type: Boolean, default: false }, // Indicates if the user has flagged the post
        ...
        shareTime: [Date], // List of absolute times when the user has share the post
        unshareTime: [Date], // List of absolute times when the user has share the post
        harmfulTime: [Date], // List of absolute times when the user has harmful the post
        unharmfulTime: [Date], // List of absolute times when the user has harmful the post
        ...
        comments: [new Schema({
            ...
            liked: { type: Boolean, default: false }, // Indicates if the user has liked the comment
            harmful: { type: Boolean, default: false }, // Indicates if the user has flagged the comment
            ...
            harmfulTime: [Date], // List of absolute times when the user has flagged the comment
            unharmfulTime: [Date], // List of absolute times when the user has unflagged the comment
            ...
            harmfuls: { type: Number, default: 0 }, // Indicates the number of likes the comment has
...
userSchema.methods.logPostStats = function logPage() {
    const counts = this.feedAction.reduce(function(newCount, feedAction) {
            ...
            const numHarmfuls = feedAction.comments.filter(comment => comment.harmful && !comment.new_comment).length;
            newCount[0] += numHarmfuls;
            newCount[0] += numLikes;
            newCount[1] += numNewComments;
            return newCount;
        }, [0, 0, 0], //[actorCommentLikesHarmfuls, newComments]
    );
    ...
    const log = {
        ...
        GeneralPostHarmfuls: this.feedAction.filter(feedAction => feedAction.harmful).length,
        GeneralCommentHarmfuls: counts[0],
        GeneralPostShares: this.feedAction.filter(feedAction => feedAction.shared).length,
