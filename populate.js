...
const posts_inputFile = './input/MergedTweets.csv';
...
console.log("OpenAI API Key:", process.env.OPENAI_API_KEY); // Print the API key
    ...
    ****/
    let promise = new Promise((resolve, reject) => { //Drop the actors collection
            ...
            ***/
            ...
            return new Promise(async (resolve, reject) => {
                ...
                for (const post of posts_list) {
                    // Get 1-3 random actors to comment on each post
                    const numComments = Math.floor(Math.random() * 3) + 1;
                    const usedActors = new Set(); // Track actors who have already commented
                    
                    for (let i = 0; i < numComments; i++) {
                        let actor;
                        do {
                            actor = await getRandomActorWithClass(post.class);
                        } while (usedActors.has(actor.username)); // Ensure no duplicate commenters
                        
                        usedActors.add(actor.username);
                        
                        const comment = await generateAIComment(post.body, actor);
                        const commentDetail = {
                            postID: post.id,
                            ...
                            time: getRandomTimeForComment(timeStringToNum(post.time)),
                            class: post.class
                        };
                ...
                console.log(color_success, `Finished generating ${comment_list.length} AI comments`);
                resolve("done");
            });
        ...
        .then(function(result) {
            ...
            /*************************
            Creates each notification(llikes, reads, shares, harmfuls) and uploads it to the DB
            Actors must be in DB first to add them correctly to the post
            *************************/
        }).then(function(result) {
            console.log(color_start, "Starting to populate notifications (likes, reads, shares, harmful) collection...");
            return new Promise((resolve, reject) => {
                async.each(notification_list, async function(new_notify, callback) {
                        ...
                        if (act) {
                            ...
                            try {
                                ...
                                console.log(color_error, "ERROR: Something went wrong with saving notification(likes, reads, shares, harmfuls) in database");
                                next(err);
                            }
                        } else { //Else no actor found
                            console.log(color_error, "ERROR: Actor not found in database");
...
//Create a random number (for the number of likes, shares, harmfuls) with a weighted distrubution
//This is for posts
function getLikes() {
...
//Create a random number (for the number of harmfuls) with a weighted distrubution
//This is for posts
function getHarmfuls() {
    var notRandomNumbers = [1, 1, 1, 2, 2, 2, 3, 3, 4, 4, 5, 6];
    var idx = Math.floor(Math.random() * notRandomNumbers.length);
    return notRandomNumbers[idx];
}
...
//Create a random number (for the number of shares) with a weighted distrubution
//This is for posts
function getShares() {
    var notRandomNumbers = [1, 1, 1, 2, 2, 2, 3, 3, 4, 4, 5, 6];
    var idx = Math.floor(Math.random() * notRandomNumbers.length);
...
// Add this function somewhere before it's used (with your other utility functions)
...
async function generateAIComment(postBody, actor) {
    try {
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                { 
                    role: "system", 
                    content: `You are a social media user with the following profile:
                        Name: ${actor.profile.name}
                        Age: ${actor.profile.age}
                        Gender: ${actor.profile.gender}
                        Bio: ${actor.profile.bio}
                        
                        Generate a comment that matches this persona.`
                },
                { 
                    role: "user", 
                    content: `Generate a realistic social media comment for this post: ${postBody}` 
                }
            ],
            max_tokens: 100
        });
        ...
        console.log(color_error, 'Error generating AI comment:', error);
        return 'Nice post!'; // fallback comment if API fails
    }
}

async function getRandomActorWithClass(postClass) {
    // Find actors that match the post's class for more realistic interactions
    const actors = await Actor.find({ class: postClass }).exec();
    if (actors.length === 0) {
        // Fallback to any actor if no matching class is found
...
function getRandomTimeForComment(postTime) {
    // Generate a random time between post time and current time
    const minTime = postTime;
