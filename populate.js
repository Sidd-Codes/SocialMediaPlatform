            ...
            ***/
            ...
            return new Promise(async (resolve, reject) => {
                ...
                for (const post of posts_list) {
                    // Generate a comment using AI based on the post content
                    const comment = await generateAIComment(post.body);
                    const commentDetail = {
                        commentID: generateCommentID(), // Generate a unique comment ID
                        body: comment,
                        likes: getLikesComment(),
                        harmfuls: getHarmfulsComment(),
                        actor: await getRandomActor(), // Pick a random actor for the comment
                        time: getRandomTimeForComment(), // Generate a time for the comment
                        class: post.class
                    };
...
async function generateAIComment(postBody) {
    try {
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                { 
                    role: "system", 
                    content: "You are a social media platform user generating comments." 
                },
                { 
                    role: "user", 
...
function getRandomTimeForComment() {
    // Generate a random time for the comment (adjust as needed)
    return Date.now();
}
